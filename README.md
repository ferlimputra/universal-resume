...
Build the production-ready CSS:

```
npm run build
```

Generate the optimized public CSS the same way as the legacy command:
...
**2.** Inside `tailwind.config.js`, uncomment code block below `/* For A4 size */`, then comment code block below `/_ For Letter size _/
